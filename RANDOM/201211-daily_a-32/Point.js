...
class Point {
  ...
  updateAfterInit() {
    ...
    // this.d = abs(this.opos.x - _grfcCenter.y) * abs(this.opos.y - _grfcCenter.x) / this.d
    this.d = floor(this.opos.x - _grfcCenter.y) * floor(this.opos.y - _grfcCenter.x)
  ...
  updateOther() {
    ...
    // const str = map(abs(this.driver - this.prevDriver), 0.0, 0.1, 0, 3, true)
    ...
    this.angle = ((this.driver - this.prevDriver) * (this.idx - (pow(_total, 2)) / 2) * 0.02)
  }
