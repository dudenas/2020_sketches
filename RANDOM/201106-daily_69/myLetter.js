...
class myLetter {
  ...
  update() {
    if (!this.finnished) {
      const fcount = (frameCount + this.idx * 0.7) % _totalFrames
      let percent = (fcount) / _totalFrames;
      // 0.25 3 // default
      // percent = ease[styles[11]](percent,0.25, 12);

      this.pos.x = map(squareWave(percent * TWO_PI), -1, 1, 0, this.opos.x * 3 , true)
      this.pos.y = map(cos(percent * TWO_PI), -1, 1, 0, this.opos.x * 1 , true)
    }
  }
