...
class Point {
  ...
  updateOther() {
    ...
    const fcount = floor(frameCount + pow(this.d, .5 / this.nidx) * atan(this.idx / _grfc.length * TWO_PI)) % (_totalFrames)
    let percent = (fcount) / (_totalFrames)
    this.size = map(cos(percent * TWO_PI), -1, 1, 1, _ratio / 2.5)
    this.pos.z = pow(this.size, this.nidx * 4)
    // const special = this.nidx * PI
    // const special = 1
    // // const fcount = floor(frameCount + this.d * asin(this.nidx * TWO_PI)) % (_totalFrames * special)
    // const fcount = floor(frameCount + this.d * atan(this.nidx * TWO_PI)) % (_totalFrames * special)
    // let percent = (fcount) / (_totalFrames * special)
