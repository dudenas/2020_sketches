...
class Point {
  ...
  update() {
    ...
    this.size = (p5.Vector.dist(this.pos, this.opos) / _ratio) + 0.5
    if (fcount == 0) {
      let nr = 1
      if (frameCount % 4 == 0) nr = 0
      this.randomDir = createRandomVector(nr)
    }
    // this.angle += map(sin(this.pos.x),-1,1, 0, PI/100 * this.nidx);
