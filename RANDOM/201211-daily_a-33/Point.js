...
class Point {
  ...
  updateAfterInit() {
    ...
    this.d = p5.Vector.dist(this.opos, _grfcCenter) * ((this.idx - pow(_total, 2) / 2)) * (abs((this.idx % _total) - _total / 2)) /
      floor(this.opos.x - _grfcCenter.x) / abs(this.opos.y - _grfcCenter.y)
    // a
    // this.d = ((this.idx - pow(_total, 3) / 2)) * (abs((this.idx % _total) - _total / 2)) /
    //   floor(this.opos.x - _grfcCenter.x) * floor(this.opos.y - _grfcCenter.y)
  ...
  updateOther() {
    ...
    // const fcount = floor(frameCount + this.idx * 150) % (_totalFrames)
    ...
    this.angle = ((this.driver - this.prevDriver) * (this.idx - (pow(_total, 2)) / 2) * 0.002)
  }
