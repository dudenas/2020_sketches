...
class myPoint {
  ...
  update(angle) {
    ...
    percent = ease[styles[9]](percent, 0.05);

    let nforce = map(openSimplex.noise2D(percent * TWO_PI, 0), -1, 1, -_r, _r)
    this.len = map(sin(percent * TWO_PI + this.idx * 0.01), -1, 1, 1, _len)

    this.r = map(cos(percent * TWO_PI), -1, 1, _r, _r / 2)
    this.pos.x = map(percent, 0, 0.5, this.opos[this.curr].x, this.opos[(this.curr + 1) % _letters.length].x, true)
    this.pos.y = map(percent, 0.5, 1, this.opos[this.curr].y, this.opos[(this.curr + 1) % _letters.length].y, true)
